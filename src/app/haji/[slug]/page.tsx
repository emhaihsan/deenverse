...
const PillarSection = ({
  ...
  data: { title: string; items: PillarOrWajib[]; notes?: string };
  icon: React.ElementType;
}) => (
  <div className="bg-white p-6 rounded-lg shadow-sm">
    <div className="flex items-center mb-4">
...
export default async function HajiDetailPage({
  params,
}: {
  params: Promise<Params>;
}) {
  const { slug } = await params; // ✅ tunggu params dulu
  const topic: HajiUmrahTopic | undefined = hajiData[slug];
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-6">
          ...
          {topic.syaratWajib && (
            <DetailSection section={topic.syaratWajib} icon={CheckSquare} />
          )}
          <DetailSection section={topic.hukum} icon={Scale} />
          {topic.perbedaanRukunWajib && (
            <DetailSection
              section={topic.perbedaanRukunWajib}
              icon={BookText}
            />
          )}
